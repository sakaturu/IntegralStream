...
interface PlaylistProps {
  ...
  isVideoShuffleMode?: boolean;
  onToggleVideoShuffle?: (on: boolean) => void;
  onOpenMusicApp?: () => void;
...
const Playlist: React.FC<PlaylistProps> = ({ 
  ...
  React.useEffect(()=>{
    if(currentVideo?.id && videoListRef.current){
      const el = videoListRef.current.querySelector(`[data-videoid="${currentVideo.id}"]`) as HTMLElement;
  ...
  return (
    <div className="flex flex-col h-full relative bg-transparent">
      <div className="flex-none pb-4 z-20 px-4 pt-6">
        <div className="flex items-center justify-between mb-4 px-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <>
            <button onClick={()=>{ if(!isUserLocked&&!isAuthorized){onRequestIdentify?.();return;} onToggleVideoShuffle?.(true); onAddRandom(); }} title="Shuffle" className={"text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-1 "+(isVideoShuffleMode?"text-purple-400":"text-slate-400 hover:text-white")}><i className="fa-solid fa-shuffle text-[11px]"></i>{isVideoShuffleMode && <span>ON</span>}</button>
            {isVideoShuffleMode && <button onClick={()=>onToggleVideoShuffle?.(false)} className="w-4 h-4 rounded-full bg-purple-500/30 text-purple-300 flex items-center justify-center hover:bg-red-500/40 hover:text-red-300 transition-all"><i className="fa-solid fa-xmark text-[8px]"/></button>}
            </>
      ...
      <div ref={videoListRef} className="flex-1 overflow-y-auto custom-scrollbar mt-2 pb-10">
        {onShowUserPlaylist ? (
