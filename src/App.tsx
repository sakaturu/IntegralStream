...
const VideoBackground: React.FC<{src: string; fallbackColor: string}> = ({src, fallbackColor}) => {
  const [failed, setFailed] = React.useState(false);
  React.useEffect(()=>{ setFailed(false); }, [src]);
  if(failed || !src) return <div style={{position:'absolute',inset:0,background:fallbackColor}}/>;
  return (
    <video autoPlay muted loop playsInline preload="auto" src={src}
      onError={()=>{ console.warn('[IS] Video failed to load:', src); setFailed(true); }}
...
const MusicApp: React.FC<MusicAppProps> = ({
  ...
  const handleAddTrack=async ()=>{
    ...
    setFormUrl('');setFormArtist('');setFormTitle('');setFormThumbnail('');setShowAddForm(false);
              ...
              <div ref={musicPlayerRef} onClick={()=>{if(currentTrackId) setIsPlaying(p=>!p);}} className="w-full max-w-[calc(100%-20px)] max-h-[calc(100vh-240px)] aspect-video bg-black rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl relative mx-auto" style={{cursor:currentTrackId?'pointer':'default'}}>
                ...
                {!currentTrackId && (
                  <div style={{position:'absolute',inset:0,zIndex:2,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:14}}>
                    <VideoBackground src="https://firebasestorage.googleapis.com/v0/b/integralstream-87a5c.firebasestorage.app/o/Default-video.mp4?alt=media&token=eb9533f2-458f-4138-99b4-d6a2eb2d17a4" fallbackColor="linear-gradient(135deg,#0d0020,#1a0040,#050015)"/>
                ...
                {currentVideo ? (
                  ...
                  {!isPlaying && (
                    <div style={{position:'absolute',inset:0,pointerEvents:'none',zIndex:2}}>
                      <VideoBackground src="https://firebasestorage.googleapis.com/v0/b/integralstream-87a5c.firebasestorage.app/o/video-section.mp4?alt=media&token=ba46fde6-91ff-43ab-bf37-6bbdf3e29cbc" fallbackColor="linear-gradient(135deg,#000814,#001233,#000408)"/>
                    </div>
                  )}
