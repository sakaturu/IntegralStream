...
const MusicApp: React.FC<MusicAppProps> = ({
  ...
  const handleShuffle=()=>{if(!filteredTracks.length)return;if(!isUserLocked&&!isAuthorized){onPendingReview();return;}const r=filteredTracks[Math.floor(Math.random()*filteredTracks.length)];setCurrentTrackId(r.id);setIsPlaying(true);};
